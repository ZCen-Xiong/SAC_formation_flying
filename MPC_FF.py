...
x = np.zeros((6, int(np.ceil(Tf/dT))+1))            # State vector
...
for t, i in zip(np.arange(0, Tf, dT) , range(0,int(np.ceil(Tf/dT)))):
    ...
    for ri in range(1, ny+1):
        t_r = t + (ri-1)*dT
        r[ri*6-6:ri*6] = CW.Free(t_r, 0, Xf, nT)
    ...
    Bcons = mpc.Dy_func(-np.ones((6,1))*4e3, np.ones((6,1))*4e3) - mpc.Acons_func()@ Mbar @ x[:, i].reshape(6, 1)
    ...
    delta_u[:, i] = delta_uFut[0:3].flatten() # 获取要应用的输入增量
